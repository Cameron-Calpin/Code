...
class FormEditor:
	def __init__(self, name, dataclass, storagedir):
		...
		self.root = root = Tk()				# create windows and size it
	...
	def add_variable(self, root, varname):
		Label(root, text=varname).grid(row=self.row, column=0, stick=E)
		value = Label(root, text='', background='gray90',
			relief=SUNKEN, anchor=W, justify=LEFT)
		value.grid(row=self.row, column=1, stick=E+W)
		self.row = self.row + 1
		return value
	...
	def select(self, event):
		...
		for fieldname in self.dataclass.fieldnames:
			label = getattr(self, fieldname)
			labelstr = getattr(self.selection, fieldname)
